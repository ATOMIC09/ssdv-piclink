...
class SSDVReceiver:
    ...
    def __init__(self, port: str, baud: int = 9600):
        ...
        self.current_batch_last_activity = time.time()  # Track when we last received a packet for current batch
        self.output_file_handle = None
        
        # Duplicate tracking for batch skipping logic
        self.batch_duplicate_counts: Dict[int, int] = {}  # batch_start -> duplicate_count
        self.duplicate_threshold = 5  # Skip to next batch after this many duplicates from same batch
    ...
    def send_batch_ack(self, batch_start: int, batch_end: int, missing_packets: list = None):
        ...
        try:
            ...
            logger.info(f"[{current_time}] RESPONSE SENT: {len(response)} bytes transmitted, raw_bytes={response.hex()}")
    ...
    def process_packet(self, sequence: int, payload: bytes) -> bool:
        ...
        if sequence in self.received_sequences:
            ...
            # Determine which batch this duplicate belongs to
            duplicate_batch_start = (sequence // self.BATCH_SIZE) * self.BATCH_SIZE
            
            # Track duplicate count for this batch
            self.batch_duplicate_counts[duplicate_batch_start] = self.batch_duplicate_counts.get(duplicate_batch_start, 0) + 1
            duplicate_count = self.batch_duplicate_counts[duplicate_batch_start]
            
            logger.warning(f"[{current_time}] DUPLICATE PACKET: seq={sequence} from batch {duplicate_batch_start//self.BATCH_SIZE}, duplicate #{duplicate_count} for this batch")
            
            # Check if we've received too many duplicates from this batch
            if duplicate_count >= self.duplicate_threshold:
                logger.warning(f"[{current_time}] EXCESSIVE DUPLICATES: received {duplicate_count} duplicates from batch {duplicate_batch_start//self.BATCH_SIZE}, transmitter likely didn't receive our ACK/NAK")
                
                # If this is from our current batch, skip to next batch
                if duplicate_batch_start == self.current_batch_start:
                    logger.warning(f"[{current_time}] BATCH SKIP: excessive duplicates from current batch {self.current_batch_start//self.BATCH_SIZE}, forcing skip to next batch")
                    self.force_skip_to_next_batch()
                # If it's from a previous batch, the transmitter is still stuck - send another NAK/ACK
                elif duplicate_batch_start < self.current_batch_start:
                    logger.warning(f"[{current_time}] RETRANSMIT RESPONSE: sending another response for previous batch {duplicate_batch_start//self.BATCH_SIZE} that transmitter seems to have missed")
                    self.resend_batch_response(duplicate_batch_start)
        ...
        # Ensure current_batch_start is initialized properly
        if self.current_batch_start == 0 and not self.current_batch_packets and sequence > 0:
            self.current_batch_start = (sequence // self.BATCH_SIZE) * self.BATCH_SIZE
            logger.info(f"[{current_time}] BATCH INIT: Initializing batch tracking at batch_start={self.current_batch_start}")
        
        # Add to batch tracking
        self.current_batch_packets.add(sequence)
        self.current_batch_last_activity = time.time()  # Update batch activity timer
        ...
        if expected_batch_start != self.current_batch_start and self.current_batch_packets:
            ...
            # Remove current packet from the batch we're about to process
            old_batch_packets = self.current_batch_packets.copy()
            old_batch_packets.discard(sequence)  # Remove current packet since it belongs to new batch
            
            # Process the old batch
            old_batch_start = self.current_batch_start
            self.current_batch_packets = old_batch_packets
            self.current_batch_start = old_batch_start
        ...
        # Check what packets we have received in the current batch
        ...
        logger.info(f"[{current_time}] BATCH STATUS: batch={self.current_batch_start//self.BATCH_SIZE}, received={batch_progress}/{batch_expected} packets, expected_packets={sorted(list(expected_packets_in_batch)[:10])}{'...' if len(expected_packets_in_batch) > 10 else ''}")
        
        # Process batch if we have reached the batch size or it's the end of transmission
        # OR if we have received enough consecutive packets to complete a batch
        # OR if we've reached the end of the current batch (even if incomplete)
        batch_end_sequence = self.current_batch_start + self.BATCH_SIZE - 1
        is_batch_end_reached = sequence == batch_end_sequence  # We've reached the last packet of this batch
        
        should_process_batch = (
            batch_progress >= self.BATCH_SIZE or  # Full batch received
            sequence >= 255 or  # End of transmission
            (batch_progress == batch_expected and batch_expected > 0) or  # All expected packets for this batch received
            is_batch_end_reached  # We've reached the end packet of the batch (even if some packets are missing)
        )
        
        if should_process_batch:
            logger.info(f"[{current_time}] BATCH COMPLETE: triggering batch processing for batch {self.current_batch_start//self.BATCH_SIZE} ({batch_progress}/{batch_expected} packets) - reason: {'full_batch' if batch_progress >= self.BATCH_SIZE else 'end_of_transmission' if sequence >= 255 else 'all_expected' if batch_progress == batch_expected else 'batch_end_reached'}")
            self.process_current_batch()
    ...
    def process_current_batch(self):
        ...
        if not self.current_batch_packets:
            logger.warning("process_current_batch called with empty current_batch_packets")
            return
        ...
        logger.info(f"[{current_time}] PROCESSING BATCH: batch {batch_start//self.BATCH_SIZE} (packets {batch_start}-{batch_end})")
        logger.info(f"[{current_time}] BATCH PACKETS: received {len(self.current_batch_packets)} packets: {sorted(list(self.current_batch_packets))}")
        ...
        for seq in expected_packets:
            if seq not in self.received_sequences:  # Check against actually received packets, not just current batch
                missing_packets.append(seq)
        
        logger.info(f"[{current_time}] BATCH ANALYSIS: expected={len(expected_packets)}, received={len(expected_packets) - len(missing_packets)}, missing={len(missing_packets)}")
        if missing_packets:
            logger.warning(f"[{current_time}] MISSING PACKETS: {missing_packets}")
        
        self.stats['batches_processed'] += 1
        
        if missing_packets:
            # Send NAK with missing packet list
            logger.warning(f"[{current_time}] SENDING NAK: batch {batch_start}-{batch_end} incomplete, missing {len(missing_packets)} packets: {missing_packets}")
            self.send_batch_ack(batch_start, batch_end, missing_packets)
        else:
            # Send ACK for complete batch
            received_in_batch = len(expected_packets) - len(missing_packets)
            logger.info(f"[{current_time}] SENDING ACK: batch {batch_start}-{batch_end} complete - all {received_in_batch} packets received")
        ...
        self.current_batch_packets = set()
        self.current_batch_last_activity = time.time()  # Reset timer for new batch
    
    def check_batch_timeout(self, current_time_seconds: float) -> bool:
        """Check if current batch has timed out and should be processed."""
        BATCH_TIMEOUT = 5.0  # 5 seconds timeout for incomplete batches
        
        if (not self.current_batch_packets or 
            current_time_seconds - self.current_batch_last_activity < BATCH_TIMEOUT):
            return False
        
        # We have packets in current batch and it's been too long since last activity
        batch_num = self.current_batch_start // self.BATCH_SIZE
        current_time = time.strftime('%H:%M:%S.%f')[:-3]
        
        logger.warning(f"[{current_time}] BATCH TIMEOUT: batch {batch_num} timed out after {BATCH_TIMEOUT}s, processing incomplete batch with {len(self.current_batch_packets)} packets")
        self.process_current_batch()
        return True

    def force_skip_to_next_batch(self):
        """Force skip to the next batch when excessive duplicates are detected."""
        if not self.current_batch_packets:
            return
            
        # Store the current batch start before processing (process_current_batch will modify it)
        original_batch_start = self.current_batch_start
        
        # Process current batch one final time to send response
        self.process_current_batch()
        
        # Calculate next batch start based on original batch
        next_batch_start = original_batch_start + self.BATCH_SIZE
        current_time = time.strftime('%H:%M:%S.%f')[:-3]
        
        logger.warning(f"[{current_time}] BATCH SKIP EXECUTED: forcing skip from batch {original_batch_start//self.BATCH_SIZE} to batch {next_batch_start//self.BATCH_SIZE}")
        
        # Reset to next batch
        self.current_batch_start = next_batch_start
        self.current_batch_packets = set()
        self.current_batch_last_activity = time.time()
        self.stats['batches_skipped'] += 1
        
        # Clear duplicate count for the skipped batch since we're moving on
        self.batch_duplicate_counts.pop(original_batch_start, None)

    def resend_batch_response(self, batch_start: int):
        """Resend the ACK/NAK response for a specific batch."""
        batch_end = min(batch_start + self.BATCH_SIZE - 1, 255)
        current_time = time.strftime('%H:%M:%S.%f')[:-3]
        
        # Find missing packets in that batch
        expected_packets = set(range(batch_start, batch_end + 1))
        missing_packets = []
        ...
        logger.info(f"[{current_time}] RESENDING RESPONSE: for batch {batch_start//self.BATCH_SIZE} ({batch_start}-{batch_end}), missing={len(missing_packets)} packets")
        
        if missing_packets:
            logger.warning(f"[{current_time}] RESENDING NAK: batch {batch_start}-{batch_end}, missing packets: {missing_packets}")
            self.send_batch_ack(batch_start, batch_end, missing_packets)
        else:
            logger.info(f"[{current_time}] RESENDING ACK: batch {batch_start}-{batch_end} was complete")
            self.send_batch_ack(batch_start, batch_end)
    ...
    def receive_ssdv_file(self, output_file: str) -> bool:
        ...
        try:
            ...
            while True:
                ...
                # Check for batch timeout (incomplete batches that should be processed)
                self.check_batch_timeout(current_time_seconds)
            ...
            if self.current_batch_packets:
                logger.info("Processing final incomplete batch before finalization")
                self.process_current_batch()
    ...
    def print_statistics(self):
        ...
        logger.info(f"Batches skipped: {self.stats['batches_skipped']}")
